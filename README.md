...
The prebuilt testers under `bin/` exercise both programs. Pick the binary matching
your container's architecture.

Server (start your server first, in another terminal):

```bash
./bin/server-tester 127.0.0.1 8000 <# of clients> <# of messages> <test #>
```

Client:

```bash
./bin/client-tester ./build/client <log prefix> 8000 <# of clients> <# of messages> <test #>
```

Both accept a `LOG_LEVEL` environment variable from `0` (most verbose) to `3`.
Level `0` produces a lot of output, so redirect it:

```bash
LOG_LEVEL=0 ./bin/server-tester 127.0.0.1 8000 100 100 5 > test.log 2>&1
```

| Test | Server tester | Client tester |
| --- | --- | --- |
| 0 | single message round-trip | client sends and prints one message |
| 1 | response format correctness | N random messages printed correctly |
| 2 | ordering, single client, 1000 messages | client terminates on type `1` |
| 3 | type `1` handling, server terminates | many clients, sustained traffic |
| 4 | 100 clients, one message each | all clients terminate cleanly |
| 5 | 100 clients × 100 messages, ordering | — |
| 6 | many clients, coordinated shutdown | — |

## Known Limitations

- `MAX_CLIENTS` is fixed at 100; there's no bounds check on the connection array
  beyond that.
- Disconnected clients aren't removed from the broadcast list, so the server keeps
  attempting sends on closed descriptors until it exits. Fine for the fuzzing
  workload, not for a long-lived service.
- `send()` return values aren't checked for short writes.
- The final broadcast is followed by a fixed `sleep(2)` before `exit(0)` to give
  clients time to drain, rather than waiting on an acknowledgement.
